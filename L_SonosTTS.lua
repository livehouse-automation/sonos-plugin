...
local LOCAL_BASE_WEB_URL = "http://%s:%d/cmh"

local SAY_TMP_FILE = "/tmp/Say.%s.%s.mp3"
local SAY_OUTPUT_FILE = "/www/cmh/Say.%s.%s"  -- Vera
-- local SAY_OUTPUT_FILE = "/etc/cmh-ludl/cmh/Say.%s.%s"	-- openLuup
local SAY_OUTPUT_URI = "%s/Say.%s.%s"
local CONCAT_EXECUTE = "cat %s > %s ; rm %s"
local DELETE_EXECUTE = "rm /www/cmh/Say.%s.*"
...
"%s/getvoice.php?t=%s&tl=%s&sv=&vn=&pitch=%s&rate=%s"]]
        ...
        if (#tableTextFragments == 0) then
            return duration, uri
        elseif (#tableTextFragments == 1) then
            returnCocde = os.execute(SAY_EXECUTE:format(file, file, RVServerURL, url.escape(tableTextFragments[1]), language, pitch, rate))
        else
            local listFiles = ""
            for i, v in ipairs(tableTextFragments)
            do
                local partFile = SAY_TMP_FILE:format(device, i)
                returnCocde = os.execute(SAY_EXECUTE:format(partFile, partFile, RVServerURL, url.escape(v), language, pitch, rate))
                listFiles = listFiles .. " " .. partFile
            end
            -- Concat the multiple MP3 files
            os.execute(CONCAT_EXECUTE:format(listFiles, file, listFiles))
...
"http://api.microsofttranslator.com/V2/Http.svc/Speak?appId=&text=%s&language=%s&format=%s&options=%s"]]
    ...
    local file = SAY_OUTPUT_FILE:format(device, "mp3")
    local returnCocde = os.execute(SAY_EXECUTE:format(file, file, token, url.escape(text), language, url.escape("audio/mp3"), url.escape(MicrosoftOption)))
    local fh = io.open(file, "a+")
    local size = fh:seek("end")
    fh:close()
...
function setup(language, engine, playFct, baseURL, googleUrl, osxUrl, maryUrl, rvURL, clientId, clientSecret, option, rate, pitch)
    ...
    if (MicrosoftOption ~= nill and MicrosoftOption:find("MaxQuality") ~= null) then
